...
def var_historic(r, level=1):
    ...
    i.e. returns the number such "level" percent of returns
...
def var_gaussian(r, level=1):
    """
    Returns the Gaussian VaR of a Series or DataFrame
    """
...
def cvar_historic(r,level=5):
    """
    Computes the Conditional Value at Risk of a Series or DataFrame
    """
...
def annualized_rets(r, periods_of_year):
    """
    Annualizes a set of returns 
    We should infer the periods per year
    but that is currently left as an excersise
    to the reader
    """
    compound_growth = (1+r).prod()
    n_periods = r.shape[0]
    return compound_growth**(periods_of_year/n_periods)-1

def annualized_vol(r, periods_of_year):
    """
    ...
    to the reader
    """
    return r.std()*(periods_of_year**0.5)

def sharpe_ratio(r,riskfree_rate, periods_of_year):
    """
    Computes the annualized sharpe ratio of a set of returns
    """
    # convert the annual riskfree rate to per period
    rf_per_period = (1+riskfree_rate)**(1/periods_of_year)-1
    excess_ret = r - rf_per_period
    ann_ex_ret = annualized_rets(excess_ret,periods_of_year)
    ann_vol = annualized_vol(r, periods_of_year)
...
def msr(riskfree_rate, er, cov):
    ...
    given the risk free rate and expected returns and a covariance matrix
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),)*n
...
def plot_ef(n_points, er, cov,show_cml=False, style='.-',riskfree_rate=0, show_ew=False, show_gmv=False):
    ...
    if show_gmv:
        ...
        ax.plot([vol_gmv],[r_gmv],color="red",marker="o",markersize=10)
    if show_cml:
        ax.set_xlim(left = 0)
